...
void ui_draw_top(const AppState* app, const EngineDoc* doc) {
    ...
    draw_text(10, 8, .45f, CY, "Pocket Compiler v0.23");
    if (doc) {
...
void ui_draw_bottom(const AppState* app, const Editor* e, const ControlState* controls, const FileExplorer* fx) {
    ...
    if (controls) {
        C2D_DrawCircleSolid((float)controls->mouse_x, (float)controls->mouse_y, 0, 3.0f, CY);
    }
    ...
    if (fx && app && app->show_file_overlay) {
        ...
        for (int i = 0; i < fx->count && i < 8; i++) {
            draw_text(34, 62 + i*14, .24f, W, fx->names[i]);
        }
    }
